...
void loop()
{
    ...
    uint8_t fwd_L_mtr_spd = 109;      // fwd spd for L motor  
	uint8_t fwd_R_mtr_spd = 110;      // fwd spd for R motor
	...
	direct_LVR = moth(L_LVR_val, R_LVR_val, 5);             // moth behavior (move to the light)
	// direct_LVR = cockroach(L_LVR_val, R_LVR_val, 5);     // cockroach behavior (seek the darkness)
	...
	switch (direction) {
		...
		case LVR_L:                        // Right
			...
			set_motors(fwd_L_mtr_spd , (fwd_R_mtr_spd + pro_LVR_spd));
			delay(15);
		break;
		case LVR_R:                        // Right
			// left mtr fwd, right mtr slow forward
			set_motors((fwd_L_mtr_spd + pro_LVR_spd), fwd_R_mtr_spd );
			delay(15);
		break;		
		default :
			stop();
...
{      
     ...
     if((dist < 15) && (dist > 0)) {            // if obj is in front of robot
        stop(); 
